(ns parrot-sdk.main
  "Kotodama WASM entrypoint for the Parrot_sdk clean-room actor (L4) — Clojure port.
  ...
  facts (`parrot_sdk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "parrot_sdk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "parrotsd_dro"
   ...
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "parrotsd_tel"
   ...
   {:entity "Payload"    :plural "payloads"    :id-prefix "parrotsd_pay"
    :fields [:droneId :type :weightKg]                     :required [:type :weightKg]
    :coerce {:weightKg :float}                  :refs {:droneId "Drone"}}
   {:entity "Swarm"      :plural "swarms"      :id-prefix "parrotsd_swa"
...
(defn emit-facts
  "EAVT facts for one record: {\"parrot_sdk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "parrot_sdk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :parrot_sdk-compat/ready)
